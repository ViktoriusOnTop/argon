...
pub async fn fetch_captcha(token: String) -> anyhow::Result<String> {
    for attempt in 0..3 {
        crate::vlog!("[mail_gw] fetch_captcha inputs: token={} attempt={}/3", token, attempt + 1);
...
async fn fetch_email(token: String) -> anyhow::Result<Value> {
    for attempt in 0..3 {
        crate::vlog!("[mail_gw] inputs: url=https://api.mail.gw/messages?page=1 attempt={}/3 bearer_token={}", attempt + 1, token);
        ...
        let response = client.get("https://api.mail.gw/messages?page=1")
            ...
            .await;
        match &response {
            Ok(res) => crate::vlog!("[mail_gw] output: http status={}", res.status()),
            Err(e) => crate::vlog!("[mail_gw] output: reqwest errored: {}", e),
        }

        if let Ok(res) = response {
            if let Ok(email_json) = res.json::<Value>().await {
                crate::vlog!("[mail_gw] output body: {}", email_json);
