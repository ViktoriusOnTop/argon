...
async fn get_domain() -> anyhow::Result<String> {
    for attempt in 0..3{
        crate::vlog!("[mail_gw] inputs: url=https://api.mail.gw/domains attempt={}/3", attempt + 1);
        ...
        let domain_json: Value = client.get("https://api.mail.gw/domains")
            ...
            .json()
            .await?;
        crate::vlog!("[mail_gw] output body: {}", domain_json);
...
async fn make_account(email: &String, body: &Value) -> anyhow::Result<String> {
    for attempt in 0..3{
        crate::vlog!("[mail_gw] inputs: url=https://api.mail.gw/accounts attempt={}/3 body={}", attempt + 1, body);
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(10))
            .build()?;
        ...
        let response_json: Value = client.post("https://api.mail.gw/accounts")
            ...
            .json()
            .await?;
        crate::vlog!("[mail_gw] output body: {}", response_json);
...
async fn get_token(id: String, body: Value) -> anyhow::Result<String> {
    for attempt in 0..3{
        crate::vlog!("[mail_gw] inputs: url=https://api.mail.gw/token attempt={}/3 id={} body={}", attempt + 1, id, body);
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(10))
            .build()?;
        ...
        let token_json: Value = client.post("https://api.mail.gw/token")
            ...
            .json()
            .await?;
        crate::vlog!("[mail_gw] output body: {}", token_json);
